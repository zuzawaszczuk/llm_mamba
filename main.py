...
def setup(finetune_name: str, ddp_setup: bool):
    if ddp_setup is True:
...
def main():
    ...
    args = parse()
    print(args)
    finetune_name = f"{args.model}-r{args.rank}-e{args.epoch}".replace("/", "_")
    
    
    setup(finetune_name, args.ddp_setup)
    ...
    # rouge_score = score_model(model, tokenizer, dataset["validation"], 256)
    ...
    rouge_score = score_model(peft_model, tokenizer, dataset["validation"], 64)
